...
export const extractChartAxis = (data: Array<Object>) =>
...
export const extractSubscriptionCategories = (data: Array<any>) => {
...
export const extractRecentData = (
  ...
  if (expenses.length || investments.length || income.length) {
    const allData = [
      ...subscriptions.map((datum) => ({ ...datum, from: 'subscriptions' })),
